...
class TestEngine2 extends JFrame implements ActionListener
{
   ...
   void showQuestion()
   {
	...
	for(int c=0;c<4;c++)
	{
	    ans[c].setText(rst.getString(c+3));
	}
	bg.clearSelection();
   ...
   void connect()
   {
	try
	{
	   ...
	   ResultSet totrow=st.executeQuery("Select count(*) from QuestionBank");
	   totrow.next();
	   totquestion=totrow.getInt(1);
	   correctans=new int[totquestion];
	   rst=st.executeQuery("select * from QuestionBank");
	   int i=0;
	   while(rst.next())
	      correctans[i++]=rst.getInt(7);
	   rst.absolute(1);
	}
	catch(Exception ex)
	{
